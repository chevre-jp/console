...
async function onClickDownload() {
    ...
    while (true) {
        ...
        if (Array.isArray(searchResult.results)) {
            datas.push(...searchResult.results.map(function (categoryCode) {
                return categoryCode2report({ categoryCode });
            }));
        }
    ...
    $.notify({
        message: datas.length + '件の区分が見つかりました',
    }, {
        type: 'primary',
        delay: 2000,
        newest_on_top: true
    ...
    const fields = [
        { label: 'コード', default: '', value: 'codeValue' },
        { label: '名称', default: '', value: 'name.ja' },
        { label: '分類名称', default: '', value: 'categoryCodeSetName' },
    ...
    const fileName = 'categoryCodes.csv';
...
function categoryCode2report(params) {
    const categoryCode = params.categoryCode;

    return {
        codeValue: String(categoryCode.codeValue),
        categoryCodeSetName: String(categoryCode.categoryCodeSetName),
        inCodeSet: categoryCode.inCodeSet,
        name: categoryCode.name,
        additionalProperty: (Array.isArray(categoryCode.additionalProperty)) ? JSON.stringify(categoryCode.additionalProperty) : ''
    };
}
