...
$(function () {
    ...
    SEARCH_URL = '/projects/' + PROJECT_ID + '/events/event/search';
    ...
    newModal.on('change', 'select[name="trip"]', function () {
    ...
    newModal.on('change', 'select[name="theater"]', _.debounce(function () {
        ...
        initializeTripSelection(theater);
    }, 500));
...
function initializeTripSelection(theater) {
    if (!theater) {
        return;
    }

    var tripSelection = $('#trip');
    tripSelection.val(null).trigger('change');
    tripSelection.select2({
        // width: 'resolve', // need to override the changed default,
        placeholder: '選択する',
        allowClear: true,
        templateSelection: function (data, container) {
        ...
        ajax: {
            cache: false,
            url: '/projects/' + PROJECT_ID + '/trips/getlist',
            dataType: 'json',
            data: function (params) {
                var query = {
                    limit: 100,
            ...
            processResults: function (data) {
                ...
                return {
                    results: data.results.map(function (eventSeries) {
                        return {
                            id: eventSeries.identifier,
                            text: eventSeries.name.ja
...
function regist() {
    ...
    var tripIdentifier = newModal.find('select[name=trip]').val();
    ...
    if (typeof theater !== 'string' || theater.length === 0
        ...
        || typeof tripIdentifier !== 'string' || tripIdentifier.length === 0
    ...
    $.ajax({
        dataType: 'json',
        url: '/projects/' + PROJECT_ID + '/events/event/new',
        type: 'POST',
        data: {
            csrfToken: csrfToken,
            theater: theater,
...
function update() {
    ...
    var tripIdentifier = editModal.find('input[name=tripIdentifier]').val();
    ...
    if (confirmed) {
        $.ajax({
            dataType: 'json',
            url: '/projects/' + PROJECT_ID + '/events/event/' + performance + '/update',
            type: 'POST',
            data: {
                theater: theater,
                screen: screen,
...
function deletePerformance() {
    ...
    if (confirmed) {
        $.ajax({
            dataType: 'json',
            url: '/projects/' + PROJECT_ID + '/events/event/' + cancellingEventId + '/cancel',
            type: 'PUT',
        }).done(function (data) {
            editModal.modal('hide');
            searchSchedule();
...
function openNewModal(token) {
    ...
    newModal.find('select[name=trip]')
    ...
    return new Vue({
        ...
        methods: {
            ...
            findCatalogByPerformance: function (performance) {
                var options = {
                    dataType: 'json',
                    url: '/projects/' + PROJECT_ID + '/events/event/' + performance.id + '/hasOfferCatalog',
                    type: 'GET',
                    data: {},
                    beforeSend: function () {
                        $('#loadingModal').modal({ backdrop: 'static' });
            ...
            findItemOfferedByPerformance: function (performance) {
                var options = {
                    dataType: 'json',
                    url: '/projects/' + PROJECT_ID + '/events/event/' + performance.id + '/itemOffered',
                    type: 'GET',
                    data: {},
                    beforeSend: function () {
                        $('#loadingModal').modal({ backdrop: 'static' });
            ...
            showPerformance: function (performance) {
                ...
                var details = $('<dl>').addClass('row')
                    ...
                    .append($('<dd>').addClass('col-md-9').append(performance.location.name.ja))
            ...
            editPerformance: function (performance, product) {
                ...
                editModal.find('input[name=theater]').val(performance.offers.itemOffered.availableChannel.serviceLocation.containedInPlace.id);
                editModal.find('input[name=screen]').val(performance.location.branchCode);
                editModal.find('input[name=day]').val(day);
                editModal.find('input[name=tripIdentifier]').val(performance.offers.itemOffered.serviceOutput.reservationFor.identifier);
                ...
                editModal.find('.film span').text(performance.name.ja);
                editModal.find('.film input').val(performance.name.ja);
                editModal.find('.theater input').val(performance.offers.itemOffered.availableChannel.serviceLocation.containedInPlace.name.ja);
            ...
            aggregateReservation: function (event) {
                ...
                $.ajax({
                    dataType: 'json',
                    url: '/projects/' + PROJECT_ID + '/events/event/' + event.id + '/aggregateReservation',
                    type: 'POST',
                }).done(function (data) {
                    alert('集計を開始しました');
                }).fail(function (jqxhr, textStatus, error) {
            ...
            postponeEvent: function (event) {
                ...
                $.ajax({
                    dataType: 'json',
                    url: '/projects/' + PROJECT_ID + '/events/event/' + event.id + '/postpone',
                    type: 'PUT',
                }).done(function (data) {
                    alert('イベントを保留しました');
                    modal.modal('hide');
            ...
            rescheduleEvent: function (event) {
                ...
                $.ajax({
                    dataType: 'json',
                    url: '/projects/' + PROJECT_ID + '/events/event/' + event.id + '/reschedule',
                    type: 'PUT',
                }).done(function (data) {
...
async function showOffersById(id) {
    ...
    while (limit === numData) {
        ...
        var offersOnPage = await new Promise((resolve, reject) => {
            $.ajax({
                dataType: 'json',
                url: '/projects/' + PROJECT_ID + '/events/event/' + id + '/offers',
                cache: false,
                type: 'GET',
                data: {
                    limit: limit,
...
function searchUpdateActionsById(id) {
    ...
    $.ajax({
        dataType: 'json',
        url: '/projects/' + PROJECT_ID + '/events/event/' + id + '/updateActions',
        cache: false,
        type: 'GET',
        data: {},
        beforeSend: function () {
...
function showOffers(event, offers, applications) {
    ...
    var url4catalog = '/projects/' + PROJECT_ID + '/events/event/' + event.id + '/showCatalog';
    ...
    modal.find('.modal-title').text('旅客');
    modal.find('.modal-body').html(div);
    modal.modal();
}
