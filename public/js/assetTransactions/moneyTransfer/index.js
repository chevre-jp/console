...
$(function () {
    ...
    $(document).on('click', '.showPendingTransaction', function (event) {
        var transactionNumber = $(this).attr('data-transactionNumber');

        showPendingTransaction(transactionNumber);
    });
});

function showPendingTransaction(transactionNumber) {
    ...
    var title = '取引 `' + assetTransaction.transactionNumber + '` pendingTransaction';

    let pendingTransaction = assetTransaction.object.pendingTransaction;
    var div = $('<div>')

    div.append($('<textarea>')
        .val(JSON.stringify(pendingTransaction, null, '\t'))
...
function showActionsByTransactionId(transactionId) {
    ...
    $.ajax({
        dataType: 'json',
        url: '/projects/' + PROJECT_ID + '/assetTransactions/moneyTransfer/' + transaction.id + '/actions',
        cache: false,
        type: 'GET',
...
function search(pageNumber) {
    ...
    var url = '/projects/' + PROJECT_ID + '/assetTransactions/moneyTransfer?format=datatable';
    $.ajax({
        dataType: 'json',
        url: url,
        cache: false,
