$(function () {
    ...
    function search(pageNumber) {
        ...
        var url = '/projects/' + PROJECT_ID + '/merchantReturnPolicies/search';
...
function showAdditionalProperty(id) {
    var returnPolicy = $.CommonMasterList.getDatas().find(function (data) {
        return data.id === id
    });
    if (returnPolicy === undefined) {
        alert('返品ポリシー' + id + 'が見つかりません');

        return;
    }

    var modal = $('#modal-merchantReturnPolicy');
    var div = $('<div>')

    if (Array.isArray(returnPolicy.additionalProperty)) {
        ...
        tbody.append(returnPolicy.additionalProperty.map(function (property) {
            return $('<tr>').append([
...
async function onClickDownload() {
    ...
    while (true) {
        ...
        if (Array.isArray(searchResult.results)) {
            datas.push(...searchResult.results.map(function (returnPolicy) {
                return returnPolicy2report({ returnPolicy });
            }));
        }
    ...
    $.notify({
        message: datas.length + '件の返品ポリシーが見つかりました',
    }, {
        type: 'primary',
        delay: 2000,
        newest_on_top: true
    ...
    const fileName = 'merchantReturnPolicies.csv';
...
function returnPolicy2report(params) {
    const returnPolicy = params.returnPolicy;

    return {
        identifier: String(returnPolicy.identifier),
        name: returnPolicy.name,
        additionalProperty: (Array.isArray(returnPolicy.additionalProperty)) ? JSON.stringify(returnPolicy.additionalProperty) : ''
    };
}
